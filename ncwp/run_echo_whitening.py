...
def load_echo(model, source_tag, split, wanted_sents):
    ...
    if not (os.path.exists(npy) and os.path.exists(sj)):
        raise FileNotFoundError(
            f"Missing {source_tag} embeddings for {model}: {npy}\n"
            f"They are produced by the prompt-baseline runner; run it first:\n"
            f"  python -m ncwp.run_prompt_baselines --model {model} --eval_split test"
        )
