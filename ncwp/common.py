"""Shared utilities for NCWP reproduction experiments."""
...
# ── Paths (repo-relative; override the root with NCWP_ROOT if needed) ──
REPO_ROOT = os.environ.get("NCWP_ROOT") or os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
STS_DIR = os.path.join(REPO_ROOT, "data", "stsbenchmark")   # bundled STS-B split JSONs
CACHE_DIR = os.path.join(REPO_ROOT, "cache", "sts")          # STS Base embeddings (built by ncwp.build_cache)
QUORA_CACHE = os.path.join(REPO_ROOT, "cache", "quora")      # BEIR-Quora Base embeddings
PROMPT_CACHE = os.path.join(REPO_ROOT, "cache", "prompt")    # prompt/pooling/layer/echo encodings
CQA_CACHE = os.path.join(REPO_ROOT, "cache", "cqa")          # CQADupStack Base embeddings (optional breadth)
OUT_ROOT = os.path.join(REPO_ROOT, "results")               # result tables (CSV/JSONL)
...
# Transformer-block counts, used to index hidden_states for the layer-selection
# analysis (index L == output of the last block).
...
def load_sts_embeddings(model: str, sts: Optional[dict] = None) -> Tuple[np.ndarray, dict, dict]:
    ...
    if not (os.path.exists(cache_emb) and os.path.exists(cache_idx)):
        raise FileNotFoundError(
            f"Missing STS Base cache for {model}: {cache_emb}\n"
            f"Build it first:  python -m ncwp.build_cache --models {model} --datasets sts"
        )
...
def load_quora_cached(model: str, fit_n: Optional[int] = None) -> dict:
    ...
    def _load(name):
        ...
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"Missing BEIR-Quora Base cache for {model}: {path}\n"
                f"Build it first:  python -m ncwp.build_cache --models {model} --datasets quora"
            )
        return np.load(path).astype(np.float32)

    corpus_np = _load("corpus_base.npy")
    query_np = _load("test_queries_base.npy")
    X_fit = _load(f"fit_query_sample_N{fit_n}.npy")
