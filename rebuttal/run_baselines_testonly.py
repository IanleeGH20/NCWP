...
Implementations are taken from the MAIN runners, not the rebuttal helpers, so the
rows are comparable to the published tables:
  ...
  ABTT    rebuttal.common fit_abtt_components / fit_abtt_pca_matrix (m=5) — ABTT is
          a rebuttal-only baseline, so there is no main-runner counterpart
...
Cached embeddings, no model loading. Output: rebuttal_outputs/table_baselines_testonly1379.
"""
import argparse
